    ...
    while(i < (unsigned int)argc)
      {
        ...
        else if(strcmp(argv[i], "-ratio") == 0)                     //  Following float is the nearest neighbor ratio threshold
        ...
        else if(strcmp(argv[i], "-maxQ") == 0)
          argtarget = PARAM_Q_LIMIT_FEATURES;
        else if(strcmp(argv[i], "-Q2sig") == 0)                     //  "Train" Matcher index on the query image and query the Signatures
          {
            _matchSig2Query = false;
            argtarget = PARAM_NONE;
          }
            ...
            switch(argtarget)
              {
                ...
                case PARAM_RATIO_THRESH:                            //  Incoming float
                  _ratioThreshold = (float)atof(argv[i]);
                  if(_ratioThreshold < MINIMUM_RATIO_THRESHOLD)
                    _ratioThreshold = MINIMUM_RATIO_THRESHOLD;
                  else if(_ratioThreshold > 1.0)
                    _ratioThreshold = 1.0;
                  argtarget = PARAM_NONE;                           //  Reset argument target
                  break;
...
bool PnPConfig::matchSig2Query() const
  {
    return _matchSig2Query;
  }
...
float PnPConfig::ratioThreshold() const
  {
...
void PnPConfig::display() const
  {
    ...
    if(_matchSig2Query)                                             //  Show me the matching direction
    ...
    cout << "Confidence:                                  " << _confidence << endl;
                                                                    //  Show me the ratio threshold
    cout << "Lowe-ratio:                                  " << _ratioThreshold << endl;
...
void PnPConfig::paramUsage() const
  {
    ...
    cout << "        -iter          Following argument is the number of RANSAC iterations to run. The default is 1000." << endl;
    cout << "        -rErr          Following argument sets the reprojection error. The default is 5 pixels." << endl;
    ...
    cout << "        -ratio         Following argument sets the ratio threshold for nearest neighbor feature matches." << endl;
    cout << "                       (Default is 0.7)." << endl;
    cout << "        -Q2sig         Build our search index from query image features and match object signatures." << endl;
    cout << "                       (The default is to do the opposite: \"train\" the index using object signatures and match query image features.)" << endl;
    ...
    cout << "                       The default is 1.0 = no scaling, but we suggest considering only 0.5 or 0.25." << endl;
