...
class Database():
	def __init__(self, **kwargs):
		...
		self.dropped_actions = {}									#  For actions that are just no good. key: (enactment, index) ==> val: True
		...
		for enactment in self.enactments:
			...
			for action_ctr in range(0, e.num_actions()):
				...
				if (enactment, action_ctr) not in self.dropped_actions:
					...
					if e.action_len(action_ctr) >= self.snippet_size:
						for snippet in e.snippets_from_action(self.snippet_size, self.stride, action_ctr):

							snippet_label       = snippet[0]		#  Label.
							snippet_start_time  = snippet[1]		#  Start time (float).
							snippet_start_frame = snippet[2]		#  Filepath to starting frame.
							snippet_end_time    = snippet[3]		#  End time (float).
							snippet_end_frame   = snippet[4]		#  Filepath to ending frame.
					...
					elif self.accept_small:
						for snippet in e.snippets_from_action(e.action_len(action_ctr), self.stride, action_ctr):
	...
	def index(self, i):
		snippets = []
		for label, actions in sorted(self.Xy.items()):
			for j in range(0, len(actions['actions'])):
				snippets.append( (label, j, actions['actions'][j]) )
		if i < len(snippets):
	...
	def actions(self):
		for label, actions in sorted(self.Xy.items()):
			...
			if label in self.original_counts:
				print(label + ': ' + str(len(reduced_set)) + ' / ' + str(self.original_counts[label]))
			else:
				print(label + ': ' + str(len(reduced_set)))
	...
	def render_snippet(self, snippet, video_name=None, color_map=None, detection_source=None):
		...
		for frame in animation_frames:
			...
			fh = open(snippet[0] + detection_source + '.txt', 'r')
			for line in fh.readlines():
				if line[0] != '#':
					arr = line.strip().split('\t')
					img_filename = arr[1]
