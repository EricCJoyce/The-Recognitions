...
class BookmarkPhase1Callback(keras.callbacks.Callback):
	...
	def on_epoch_end(self, epoch, logs=None):
		...
		fh.close()
		fh = open('bookmark_ph1.backup.txt', 'w')
		for line in lines:
			fh.write(line)
		fh.close()
		fh = open('bookmark_ph1.txt', 'w')
		...
		fh = open('bookmark_ph1.log', 'r')							#  The log is for plotting after training:
		lines = fh.readlines()										#  track the curve
		fh.close()
		fh = open('bookmark_ph1.backup.log', 'w')
		for line in lines:
			fh.write(line)
		fh.close()
		fh = open('bookmark_ph1.log', 'w')
...
class BookmarkPhase2Callback(keras.callbacks.Callback):
	...
	def on_epoch_end(self, epoch, logs=None):
		...
		lines = fh.readlines()										#  how many epochs have we done, and what's the loss so far?
		fh.close()
		fh = open('bookmark_ph2.backup.txt', 'w')
		for line in lines:
			fh.write(line)
		fh.close()
		fh = open('bookmark_ph2.txt', 'w')
		...
		fh = open('bookmark_ph2.log', 'r')							#  The log is for plotting after training:
		lines = fh.readlines()										#  track the curve
		fh.close()
		fh = open('bookmark_ph2.backup.log', 'w')
		for line in lines:
			fh.write(line)
		fh.close()
		fh = open('bookmark_ph2.log', 'w')
...
def main():
	...
	if len(params['enactments']) == 0:								#  No enactments listed? Use all *_groundtruth.txt that exist.
		for filename in [f for f in os.listdir('.') if os.path.isfile(f) and f.endswith('_groundtruth.txt')]:
	...
	for enactment in params['enactments']:
		...
		for line in lines:
			if line[0] != '#':
				arr = line.strip().split('\t')						#  timestamp, filename, label, object-class-name, score, BBox-x1,BBox-y1;BBox-x2,BBox-y2, mask-filename
				if arr[3] not in params['ignore'] and arr[3] != '*':
					classes[ arr[3] ] = True
...
def train_model(train, valid, classes, params):
	...
	if params['init_with'] == "imagenet":
		model.load_weights(model.get_imagenet_weights(), by_name=True)
	elif params['init_with'] == "coco":
		# Load weights trained on MS COCO, but skip layers that
		# are different due to the different number of classes
		# See README for instructions to download the COCO weights
		model.load_weights(COCO_MODEL_PATH, by_name=True, exclude=["mrcnn_class_logits", "mrcnn_bbox_fc", "mrcnn_bbox", "mrcnn_mask"])
	elif params['init_with'] == "last":
