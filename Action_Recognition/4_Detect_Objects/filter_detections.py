...
def main():
	...
	for enactment_name in params['enactments']:
		...
		format_string = '{0:.' + str(params['precision']) + 'f}'

		detection_dst_file = enactment_name + params['source-suffix'].split('-')[0] + '-th' + format_string.format(params['score-threshold']) + '_detections.txt'
		detection_directory_dst = enactment_name + params['source-suffix'].split('-')[0] + '-th' + format_string.format(params['score-threshold'])
...
def get_command_line_params():
	...
	params['precision'] = 2											#  Number of decimal places to include in the new file names.
	params['verbose'] = False
	params['helpme'] = False

	argtarget = None												#  Current argument to be set
																	#  Permissible setting flags
	flags = ['-e', '-src', '-th', '-minpx', '-prec', \
	         '-v', '-?', '-help', '--help']
		...
		if sys.argv[i] in flags:
			...
			if argtarget is not None:
				if argtarget == '-e':
					...
					params['precision'] = max(1, int(argval))
...
def usage():
	...
	print('                 Default is 0.6.')
	print('        -minpx   Following integer in [1, inf) is the minimum bounding box area in pixels for a detection to count.')
	print('                 Default is 1.')
	print('        -prec    Following integer in [1, inf) is the number of decimal places to include in the new file names.')
	print('                 Default is 2.')
	print('')
