...
class Classifier():
	...
	def initialize_stats(self):
		...
		classification_stats['_matches'] = []						### Matching information
																	#  key:_matches ==> val: [ ( prediction, ground-truth,
																	#                            DB-index,
	...
	def write_detailed_matches(self, stats, file_timestamp=None):
		...
		fh.write('#    Pred.    G.T.    DB-Index    Query-Enactment    Query-Start-Timecode(incl.)    Query-End-Timecode(excl.)    Query-Indices    Template-Indices    fair/unfair\n')
		for match in stats['_matches']:
			if match[0] is None:									#  Write Prediction.
				fh.write('*\t')
			else:
				fh.write(match[0] + '\t')
			if match[1] is None:									#  Write Ground-Truth.
				fh.write('*\t')
			else:
				fh.write(match[1] + '\t')
			fh.write(str(match[2]) + '\t')							#  Write Database Index.
																	#  Write Query Enactment and start (inclusive) and end (exclusive) time stamps.
			fh.write(match[3] + '\t' + str(match[4]) + '\t' + str(match[5]) + '\t')
			fh.write(' '.join([str(x) for x in match[6]]) + '\t')	#  Write space-separated string of Query Frame Indices
			fh.write(' '.join([str(x) for x in match[7]]) + '\t')	#  Write space-separated string of Template Frame Indices
			if match[8]:
				fh.write('fair\n')
			else:
				fh.write('unfair\n')
