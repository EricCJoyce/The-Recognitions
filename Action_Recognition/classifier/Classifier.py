...
class Classifier():
	...
	def classify(self, query):
		...
		#############################################################
			...
			if self.prediction_source == 'knn':
				...
				confidences[ nearest_neighbor_label ] *= np.mean( [self.w_train[x] for x in support_indices[nearest_neighbor_label]] )
	...
	def write_evaluation_scores(self, stats, file_timestamp=None):
		...
		#prediction_full_seq   = [ stats['_tests'][i][0] for i in range(0, len(stats['_tests'])) ]
		#ground_truth_full_seq = [ stats['_tests'][i][1] for i in range(0, len(stats['_tests'])) ]
	...
	def modified_overlap_score(self, pred_full_seq, gt_full_seq):
		...
		for i in range(0, len(ground_truth_segment_labels)):		#  For every ground-truth segment...
			#gt_label    = ground_truth_segment_labels[i]
	...
	def segmental_edit_score(self, pred_full_seq, gt_full_seq):
		pred_seq = self.prediction_segment(pred_full_seq)			#  Compute the collapsed sequences.
		gt_seq = self.ground_truth_segment(gt_full_seq)

		edit_distance = self.levenshtein_distance(pred_seq, gt_seq)	#  Compute edit distance.
