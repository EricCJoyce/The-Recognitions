...
#define L1_DIST 1                                                   /* Build the cost-matrix using L1 distances. */
#define L2_DIST 2                                                   /* Build the cost-matrix using L2 distances. */

/*
#define __DTW_DEBUG 1
*/

unsigned int build_L1_matrix(double*, unsigned int, double*, unsigned int, unsigned int, double**);
...
/* Given two matrices, Q in Real^q_len-by-d, T in Real^t_len-by-d, compute the L1 distance between
...
unsigned int build_L1_matrix(double* Q, unsigned int q_len,
            ...
            for(i = 0; i < d; i++)                                  //  For every element in the two vectors...
              {
                ...
                accum += fabs(diff);                                //  and add the absolute value of differences to the accumulator.
              }
            (*C)[y * t_len + x] = accum;                            //  C stores row-major.
          }
      }
...
static PyObject* L1(PyObject* Py_UNUSED(self), PyObject* args)
  {
    ...
    build_L1_matrix(query, query_len, template, template_len, d, &C);
...
static PyObject* path(PyObject* Py_UNUSED(self), PyObject* args)
  {
    ...
    PyTuple_SetItem(ret, 2, PyTuple_New(pathLen));                  //  Set element two: a tuple to contain Query-equivalent frame indices--
    for(i = 0; i < pathLen; i++)                                    //                   that is, indices INTO THE TEMPLATE.
      PyTuple_SetItem(PyTuple_GetItem(ret, 2), i, PyLong_FromLong(alignment_b[i]));

    PyTuple_SetItem(ret, 3, PyTuple_New(pathLen));                  //  Set element three: a tuple to contain Template-equivalent frame indices--
    for(i = 0; i < pathLen; i++)                                    //                     that is, indices INTO THE QUERY.
      PyTuple_SetItem(PyTuple_GetItem(ret, 3), i, PyLong_FromLong(alignment_a[i]));
...
/* This function handles the tasks of both L2()/L1() and path() in one call. */
static PyObject* DTW(PyObject* Py_UNUSED(self), PyObject* args)
  {
    ...
    double diagonal;                                                //  Weight for moving diagonally across the cost matrix.
    unsigned char ltype = L2_DIST;                                  //  Default to L2 distance.
    double* query;                                                  //  The query as a row-major array, frames-by-dimensionality.
    unsigned int query_len;                                         //  Length of the outer list of Q.
    double* template;                                               //  The template as a row-major array, frames-by-dimensionality.
    unsigned int template_len;                                      //  Length of the outer list of T.
    ...
    if(!PyArg_ParseTuple(args, "O!O!db", &PyList_Type, &Q, &PyList_Type, &T, &diagonal, &ltype))
      return NULL;
...
static PyMethodDef methods[] =
  {
    ...
    {"L1", &L1, METH_VARARGS, "Compute the cost matrix between all frames using L1 distance.\nInput: two lists of tuples of floats, Q and T. The number of columns must be the same.\nOutput: a matrix with as many rows as Q has frames and as many columns as T has frames."},
